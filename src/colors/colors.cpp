...
uint32_t GenerateSolidColor::get_color(const uint16_t index,
                                       const uint16_t maxIndex) const {
  return _color;
}

uint32_t GenerateRainbowColor::get_color(const uint16_t index,
                                         const uint16_t maxIndex) const {
  const uint16_t hue = map(index, 0, maxIndex, 0, 360);
  return utils::hue_to_rgb_sinus(hue);
}

uint32_t GenerateGradientColor::get_color(const uint16_t index,
                                          const uint16_t maxIndex) const {
  return utils::get_gradient(_colorStart, _colorEnd, index / (float)maxIndex);
}

uint32_t GenerateRoundColor::get_color(const uint16_t index,
                                       const uint16_t maxIndex) const {
...
uint32_t GenerateRainbowSwirl::get_color(const uint16_t index,
                                         const uint16_t maxIndex) const {
...
uint32_t GeneratePaletteStep::get_color(const uint16_t index,
                                        const uint16_t maxIndex) const {
  return get_color_from_palette(_index, *_paletteRef);
}

uint32_t GeneratePaletteIndexed::get_color(const uint16_t index,
                                           const uint16_t maxIndex) const {
  return get_color_from_palette(_index, *_paletteRef);
}

uint32_t GenerateRainbowPulse::get_color(const uint16_t index,
                                         const uint16_t maxIndex) const {
  return LedStrip::ColorHSV(_currentPixelHue);
}

uint32_t GenerateRainbowIndex::get_color(const uint16_t index,
                                         const uint16_t maxIndex) const {
  return LedStrip::ColorHSV(_currentPixelHue);
}

uint32_t GeneratePastelPulse::get_color(const uint16_t index,
                                        const uint16_t maxIndex) const {
