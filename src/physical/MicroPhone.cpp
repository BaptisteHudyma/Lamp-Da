...
void fftDisplay(const uint8_t speed, const uint8_t scale,
  ...
  for (int x = 0; x < cols; x++) {
    ...
    if (NUM_BANDS < 16)
      band = map(band, 0, NUM_BANDS - 1, 0,
                 15);  // always use full range. comment out this line to get
                       // the previous behaviour.
    band = constrain(band, 0, 15);
    uint16_t barHeight = map(fftResult[band], 0, 255, 0,
                             rows);  // do not subtract -1 from rows here
...
void mode_ripplepeak(const uint8_t rippleNumber, const palette_t& palette,
                     LedStrip& strip) {  // * Ripple peak. By Andrew Tuline.
...
#define maxsteps 16  // Case statement wouldn't allow a variable.
  ...
  strip.fadeToBlackBy(
      240);  // Lower frame rate means less effective fading than FastLED
  ...
  for (int i = 0; i < rippleCount; i++) {  // Limit the number of ripples.
    ...
    switch (ripples[i].state) {
      ...
      case 0:
        ...
        strip.setPixelColor(ripples[i].pos,
                            utils::color_blend(cFond, c, fadeLevel));
        ripples[i].state++;
        break;
      ...
      default:  // Middle of the ripples.
        ...
        c.color = get_color_from_palette(ripples[i].color, palette);
        strip.setPixelColor(
            (ripples[i].pos + ripples[i].state + LED_COUNT) % LED_COUNT,
            utils::color_blend(cFond, c, fadeLevel / ripples[i].state * 2));
        strip.setPixelColor(
            (ripples[i].pos - ripples[i].state + LED_COUNT) % LED_COUNT,
            utils::color_blend(cFond, c, fadeLevel / ripples[i].state * 2));
...
void mode_2DWaverly(const uint8_t speed, const uint8_t scale,
  ...
  for (int i = 0; i < cols; i++) {
    ...
    thisVal *=
        max(10.0, volumeSmth);  // set a min size to always have the visual
    thisVal = constrain(thisVal, 0, 512);
    uint16_t thisMax = map(thisVal, 0, 512, 0, rows / 2);
    for (int j = 0; j < thisMax; j++) {
      const auto color =
          get_color_from_palette((uint8_t)map(j, 0, thisMax, 250, 0), palette);
