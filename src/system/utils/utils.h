 ...
 */
template <typename T>
static constexpr uint32_t hash(const T s, const uint16_t maxSize = 12,
...
static constexpr uint32_t hash(const char (&s)[N]) {
  static_assert((N - 1 <= 12) &&
                "Use hash(s, maxSize) to hash strings longer than 12 bytes!");
  return hash(s, 12);
}
...
constexpr uint16_t voltageToAnalogRead(const float voltage) {
  return constrain(voltage, 0, internalReferenceVoltage) * ADC_MAX_VALUE /
         internalReferenceVoltage;
}

// convert a liion battery level to a linear model
constexpr uint16_t liion_level_to_battery_percent(
    const uint16_t liionLevelPercent) {
...
constexpr uint16_t get_battery_level_percent(const uint16_t batteryLevel) {
  return liion_level_to_battery_percent(constrain(
      map(batteryLevel, batteryMinVoltage_mV, batteryMaxVoltage_mV, 0, 10000),
      0, 10000));
}

constexpr uint16_t get_battery_max_safe_level() {
  return get_battery_level_percent(batteryMaxVoltageSafe_mV);
}

constexpr uint16_t get_battery_min_safe_level() {
...
constexpr uint16_t get_battery_level(const uint16_t batteryVoltage_mV) {
  ...
  return constrain(map(batteryVoltage_mV, get_battery_min_safe_level(),
                       get_battery_max_safe_level(), 0, 10000),
                   0, 10000);
}
