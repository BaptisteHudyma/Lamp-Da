...
void pd_dfp_pe_init(int port) { memset(&pe[port], 0, sizeof(struct pd_policy)); }

static void dfp_consume_identity(int port, int cnt, uint32_t* payload)
{
  int ptype = PD_IDH_PTYPE(payload[VDO_I(IDH)]);
  size_t identity_size = MIN(sizeof(pe[port].identity), (cnt - 1) * sizeof(uint32_t));
  pd_dfp_pe_init(port);
...
static void dfp_consume_modes(int port, int cnt, uint32_t* payload)
{
  ...
  {
    memcpy(pe[port].svids[pe[port].svid_idx].mode_vdo, &payload[1], sizeof(uint32_t) * pe[port].svids[idx].mode_cnt);
  }
...
int allocate_mode(int port, uint16_t svid)
{
  ...
  for (i = 0; i < supported_modes_cnt; i++)
  {
    ...
    for (j = 0; j < pe[port].svid_cnt; j++)
    {
      ...
      if ((svidp->svid != supported_modes[i].svid) || (svid && (svidp->svid != svid)))
        continue;
...
static int validate_mode_request(struct svdm_amode_data* modep, uint16_t svid, int opos)
{
  if (!modep->fx)
    return 0;
...
uint16_t pd_get_identity_vid(int port) { return PD_IDH_VID(pe[port].identity[0]); }

uint16_t pd_get_identity_pid(int port) { return PD_PRODUCT_PID(pe[port].identity[2]); }

#ifdef CONFIG_CMD_USB_PD_PE
static void dump_pe(int port)
{
  const char* const idh_ptype_names[] = {"UNDEF", "Hub", "Periph", "PCable", "ACable", "AMA", "RSV6", "RSV7"};
  ...
  for (i = 0; i < pe[port].svid_cnt; i++)
  {
    ...
    for (j = 0; j < pe[port].svids[j].mode_cnt; j++)
      ccprintf(" [%d] %08x", j + 1, pe[port].svids[i].mode_vdo[j]);
    ccprintf("\n");
    modep = get_modep(port, pe[port].svids[i].svid);
    if (modep)
    {
      mode_caps = modep->data->mode_vdo[modep->opos - 1];
      ccprintf("MODE[%d]: svid:%04x caps:%08x\n", modep->opos, modep->fx->svid, mode_caps);
    }
  }
}
...
#ifdef CONFIG_CHARGE_MANAGER
        if (pd_charge_from_device(pd_get_identity_vid(port), pd_get_identity_pid(port)))
...
int pd_svdm(int port, int cnt, uint32_t* payload, uint32_t** rpayload) { return 0; }
...
{
#if defined(CONFIG_USB_PD_ALT_MODE) && !defined(CONFIG_USB_PD_ALT_MODE_DFP) && !defined(CONFIG_USB_PD_SIMPLE_DFP) && \
        defined(CONFIG_USB_BOS)
