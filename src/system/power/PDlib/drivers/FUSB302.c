 ...
 */
static void fusb302_pd_reset(int port) { tcpc_write(port, TCPC_REG_RESET, TCPC_REG_RESET_PD_RESET); }
...
static int fusb302_send_message(int port, uint16_t header, const uint32_t* data, uint8_t* buf, int buf_pos)
{
  int rv;
  int reg;
  int len;
...
static int fusb302_tcpm_release(int port) { return EC_ERROR_UNIMPLEMENTED; }
...
static int fusb302_rx_fifo_is_empty(int port)
{
  ...
  ret = (!tcpc_read(port, TCPC_REG_STATUS1, &reg)) && (reg & TCPC_REG_STATUS1_RX_EMPTY);
...
static int fusb302_tcpm_transmit(int port, enum tcpm_transmit_type type, uint16_t header, const uint32_t* data)
{
  /*
