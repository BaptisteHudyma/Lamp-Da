...
struct PD_msg_state_t {
  const char *name;
  void (*handler)(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                  PD_protocol_event_t *events);
...
#define SET_MSG_STAGE(d, s)                         \
  do {                                              \
    static struct PD_msg_state_t m;                 \
    memcpy_P(&m, s, sizeof(struct PD_msg_state_t)); \
    d = &m;                                         \
  } while (0)
#define SET_MSG_NAME(d, s) \
  do {                     \
    static char n[16];     \
    strncpy_P(n, s, 15);   \
    d = n;                 \
...
static void handler_good_crc(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                             PD_protocol_event_t *events);
static void handler_goto_min(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                             PD_protocol_event_t *events);
static void handler_accept(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                           PD_protocol_event_t *events);
static void handler_reject(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                           PD_protocol_event_t *events);
static void handler_ps_rdy(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                           PD_protocol_event_t *events);
static void handler_source_cap(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                               PD_protocol_event_t *events);
static void handler_BIST(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                         PD_protocol_event_t *events);
static void handler_alert(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                          PD_protocol_event_t *events);
static void handler_vender_def(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                               PD_protocol_event_t *events);
static void handler_PPS_Status(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                               PD_protocol_event_t *events);

static bool responder_get_sink_cap(PD_protocol_t *p, uint16_t *header,
                                   uint32_t *obj);
static bool responder_reject(PD_protocol_t *p, uint16_t *header, uint32_t *obj);
static bool responder_soft_reset(PD_protocol_t *p, uint16_t *header,
                                 uint32_t *obj);
static bool responder_source_cap(PD_protocol_t *p, uint16_t *header,
                                 uint32_t *obj);
static bool responder_vender_def(PD_protocol_t *p, uint16_t *header,
                                 uint32_t *obj);
static bool responder_sink_cap_ext(PD_protocol_t *p, uint16_t *header,
                                   uint32_t *obj);
static bool responder_not_support(PD_protocol_t *p, uint16_t *header,
                                  uint32_t *obj);
...
static const struct PD_msg_state_t ctrl_msg_list[] PROGMEM = {
    ...
    {.name = str_Get_Sink_Cap,
     .handler = 0,
     .responder = responder_get_sink_cap},
    ...
    {.name = str_Get_PPS_Stat,
     .handler = 0,
     .responder = responder_not_support},
    {.name = str_Get_CC, .handler = 0, .responder = responder_not_support},
    {.name = str_Get_Sink_Ext,
     .handler = 0,
     .responder = responder_sink_cap_ext},
...
static const struct PD_msg_state_t data_msg_list[] PROGMEM = {
    ...
    {.name = str_Src_Cap,
     .handler = handler_source_cap,
     .responder = responder_source_cap},
    ...
    {.name = str_VDM,
     .handler = handler_vender_def,
     .responder = responder_vender_def},
...
static const struct PD_msg_state_t ext_msg_list[] PROGMEM = {
    ...
    {.name = str_Get_Bat_Stat,
     .handler = 0,
     .responder = responder_not_support},
    {.name = str_Bat_Cap, .handler = 0, .responder = 0},
    {.name = str_Get_Mfg_Info,
     .handler = 0,
     .responder = responder_not_support},
...
static const PD_power_option_setting_t power_option_setting[8] = {
    {.limit = 25,
     .use_voltage = 1,
     .use_current = 0}, /* PD_POWER_OPTION_MAX_5V */
    {.limit = 45,
     .use_voltage = 1,
     .use_current = 0}, /* PD_POWER_OPTION_MAX_9V */
    {.limit = 60,
     .use_voltage = 1,
     .use_current = 0}, /* PD_POWER_OPTION_MAX_12V */
    {.limit = 75,
     .use_voltage = 1,
     .use_current = 0}, /* PD_POWER_OPTION_MAX_15V */
    {.limit = 100,
     .use_voltage = 1,
     .use_current = 0}, /* PD_POWER_OPTION_MAX_20V */
    {.limit = 100,
     .use_voltage = 1,
     .use_current = 0}, /* PD_POWER_OPTION_MAX_VOLTAGE */
    {.limit = 125,
     .use_voltage = 0,
     .use_current = 1}, /* PD_POWER_OPTION_MAX_CURRENT */
    {.limit = 12500,
     .use_voltage = 1,
     .use_current = 1}, /* PD_POWER_OPTION_MAX_POWER */
};

static uint8_t evaluate_src_cap(PD_protocol_t *p, uint16_t PPS_voltage,
                                uint8_t PPS_current) {
  ...
  /* If selected option is not available, use first PDO. Reference: 6.4.1
     Capabilities Message The vSafe5V Fixed Supply Object Shall always be the
     first object. */
  if (option >=
      sizeof(power_option_setting) / sizeof(power_option_setting[0])) {
    return 0;
  }
  ...
  for (uint8_t n = 0; PD_protocol_get_power_info(p, n, &info); n++) {
    if (info.type == PD_PDO_TYPE_AUGMENTED_PDO) {
      ...
      if (info.min_v * 5 <= pps_v && pps_v <= info.max_v * 5 &&
          pps_i <= info.max_i) {
        return n;
      }
    } else {
      ...
      uint16_t power = (uint16_t)v * i; /* reduce 10-bit power info to 8-bit and
                                           use 8-bit x 8-bit multiplication */
...
static uint16_t generate_header(PD_protocol_t *p, uint8_t type,
                                uint8_t obj_count) {
  /* Reference: 6.2.1.1 Message Header */
  uint16_t h =
      ((uint16_t)type << 0) | /*   4...0  Message Type */
      ((uint16_t)PD_SPECIFICATION_REVISION
       << 6) |                         /*   7...6  Specification Revision */
...
static uint16_t generate_header_ext(PD_protocol_t *p, uint8_t type,
                                    uint8_t data_size, uint32_t *obj) {
  uint16_t h = generate_header(
      p, type,
      (data_size + 5) >> 2); /* set obj_count to fit ext header and data */
...
static void handler_good_crc(PD_protocol_t *p, uint16_t header, uint32_t *obj,
  ...
  /* Reference: 6.2.1.3 Message ID
     MessageIDCounter Shall be initialized to zero at power-on / reset,
     increment when receive GoodCRC Message */
...
static void handler_source_cap(PD_protocol_t *p, uint16_t header, uint32_t *obj,
  ...
  p->power_data_obj_selected =
      evaluate_src_cap(p, p->PPS_voltage, p->PPS_current);
  if (events) {
...
static bool responder_get_sink_cap(PD_protocol_t *p, uint16_t *header,
  ...
  uint32_t data =
      ((uint32_t)100 << 0) |  /* B9...0     Operational Current in 10mA units */
  ...
  *obj = data; /* Only implement 5V 1A Fix supply PDO. Source rarely request
                  sink cap */
...
/* Reference: 6.5.13 Sink_Capabilities_Extended Message
              6.12.3 Applicability of Extended Messages  (Normative; Shall be
   supported) */
#define SINK_CAP_VID 0
#define SINK_CAP_PID 0
#define SINK_CAP_XID \
  0 /* If the vendor does not have an XID, then it Shall return zero */
#define SINK_CAP_FW_Version 1
#define SINK_CAP_HW_Version 1
#define SINK_CAP_SKEDB_Version 1
#define SINK_CAP_SINK_MODE \
  0x3 /* Bit 0: PPS charging supported, Bit 1: VBUS powered */
...
#define SINK_CAP_SINK_MAX_PDP 100 /* Maximum     PD Power in Watt */
  static const uint32_t SKEDB[6] PROGMEM = {
      /* 2-byte header + 21-byte data, chunked to 6 PDO */
      /* PDO[0], data byte  0...1  */
      ...
      ((uint32_t)SINK_CAP_SINK_MIN_PDP << 0) | /* Byte     18  Minimum PDP */
          ((uint32_t)SINK_CAP_SINK_OP_PDP
           << 8) | /* Byte     19  Operational PDP */
...
static bool responder_reject(PD_protocol_t *p, uint16_t *header,
                             uint32_t *obj) {
...
static bool responder_source_cap(PD_protocol_t *p, uint16_t *header,
  ...
  if (info.type == PD_PDO_TYPE_AUGMENTED_PDO) {
    /* NOTE: To compatible PD2.0 PHY, do not set Unchunked Extended Messages
     * Supported */
    data = ((uint32_t)p->PPS_current
            << 0) | /* B6 ...0    Operating Current 50mA units */
           ((uint32_t)p->PPS_voltage
            << 9) |               /* B19...9    Output Voltage in 20mV units */
           ((uint32_t)1 << 25) |  /* B25        USB Communication Capable */
           ((uint32_t)pos << 28); /* B30...28   Object position (000b is
                                     Reserved and Shall Not be used) */
  } else {
    uint32_t req = info.max_i ? info.max_i : info.max_p;
    data = ((uint32_t)req << 0) | /* B9 ...0    Max Operating Current 10mA units
...
void PD_protocol_handle_msg(PD_protocol_t *p, uint16_t header, uint32_t *obj,
                            PD_protocol_event_t *events) {
...
void PD_protocol_create_request(PD_protocol_t *p, uint16_t *header,
                                uint32_t *obj) {
  responder_source_cap(p, header, obj);
}

bool PD_protocol_get_power_info(PD_protocol_t *p, uint8_t index,
  ...
  if (p && index < p->power_data_obj_count && power_info) {
    ...
    switch (power_info->type) {
      case PD_PDO_TYPE_FIXED_SUPPLY:
        ...
        power_info->min_v = 0;
        power_info->max_v =
            (obj >> 10) & 0x3FF; /*  B19...10  Voltage in 50mV units */
        power_info->max_i =
            (obj >> 0) & 0x3FF; /*  B9 ...0   Max Current in 10mA units */
        power_info->max_p = 0;
        break;
      case PD_PDO_TYPE_BATTERY:
        /* Reference: 6.4.1.2.5 Battery Supply Power Data Object */
        power_info->min_v =
            (obj >> 10) & 0x3FF; /*  B19...10  Min Voltage in 50mV units */
        power_info->max_v =
            (obj >> 20) & 0x3FF; /*  B29...20  Max Voltage in 50mV units */
        power_info->max_i = 0;
        power_info->max_p =
            (obj >> 0) &
            0x3FF; /*  B9 ...0   Max Allowable Power in 250mW units */
        break;
      ...
      case PD_PDO_TYPE_AUGMENTED_PDO:
        /* Reference: 6.4.1.3.4 Programmable Power Supply Augmented Power Data
         * Object */
        power_info->max_v = ((obj >> 17) & 0xFF) *
                            2; /*  B24...17  Max Voltage in 100mV units */
        power_info->min_v =
            ((obj >> 8) & 0xFF) * 2; /*  B15...8   Min Voltage in 100mV units */
        power_info->max_i =
            ((obj >> 0) & 0x7F) * 5; /*  B6 ...0   Max Current in 50mA units */
        power_info->max_p = 0;
        break;
    }
    return true;
...
bool PD_protocol_get_msg_info(uint16_t header, PD_msg_info_t *msg_info) {
  ...
  if (msg_info) {
    ...
    SET_MSG_STAGE(state, header & 0x8000 ? &ext_msg_list[type]
                         : h.num_of_obj  ? &data_msg_list[type]
                                         : &ctrl_msg_list[type]);
...
bool PD_protocol_get_PPS_status(PD_protocol_t *p, PPS_status_t *PPS_status) {
  if (p && PPS_status) {
    ...
    PPS_status->flag_PTF =
        (PPS_PTF_t)((p->PPSSDB[3] >> 1) & 0x3); /* Bit 1 ... 2 */
...
bool PD_protocol_set_power_option(PD_protocol_t *p,
                                  enum PD_power_option_t option) {
...
bool PD_protocol_set_PPS(PD_protocol_t *p, uint16_t PPS_voltage,
                         uint8_t PPS_current, bool strict) {
