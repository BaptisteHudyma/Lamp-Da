...
FUSB302_ret_t FUSB302_read_vbus_level(FUSB302_dev_t* dev, uint16_t* vbusVoltage)
{
  uint8_t mdac = 0;
  for (int i = 5; i >= 0; i--)
  {
    uint8_t bitI = 1UL << i;
    if (read_vbus_bit(dev, mdac | bitI))
      mdac |= bitI;
  }
...
{
  FUSB302_ret_t (*const handler[])(FUSB302_dev_t*, FUSB302_event_t*) = {FUSB302_state_unattached,
                                                                        FUSB302_state_attached};
