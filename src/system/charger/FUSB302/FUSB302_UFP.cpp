...
#define PWR_RECEIVER \
  (0x01 << 1) /* Receiver powered and current reference for Measure block */
...
enum FUSB302_state_t { FUSB302_STATE_UNATTACHED = 0, FUSB302_STATE_ATTACHED };

#define FUSB302_ERR_MSG(s) s

#define REG_READ(addr, data, count)                            \
  do {                                                         \
    if (reg_read(dev, addr, data, count) != FUSB302_SUCCESS) { \
      return FUSB302_ERR_READ_DEVICE;                          \
    }                                                          \
  } while (0)

#define REG_WRITE(addr, data, count)                            \
  do {                                                          \
    if (reg_write(dev, addr, data, count) != FUSB302_SUCCESS) { \
      return FUSB302_ERR_WRITE_DEVICE;                          \
    }                                                           \
  } while (0)

static inline FUSB302_ret_t reg_read(FUSB302_dev_t *dev, uint8_t address,
...
static inline FUSB302_ret_t reg_write(FUSB302_dev_t *dev, uint8_t address,
                                      uint8_t *data, uint8_t count) {
...
static FUSB302_ret_t FUSB302_read_incoming_packet(FUSB302_dev_t *dev,
                                                  FUSB302_event_t *events) {
  uint8_t len, b[3];
  REG_READ(ADDRESS_FIFOS, b, 3);
  dev->rx_header = ((uint16_t)b[2] << 8) | b[1];
  len = (dev->rx_header >> 12) & 0x7;
  REG_READ(ADDRESS_FIFOS, dev->rx_buffer,
           len * 4 + 4); /* add 4 to len to read CRC out */
...
static FUSB302_ret_t FUSB302_state_unattached(FUSB302_dev_t *dev,
                                              FUSB302_event_t *events) {
  REG_READ(ADDRESS_STATUS0, &REG_STATUS0, 1);
...
static FUSB302_ret_t FUSB302_state_attached(FUSB302_dev_t *dev,
                                            FUSB302_event_t *events) {
...
FUSB302_ret_t FUSB302_init(FUSB302_dev_t *dev) {
  ...
  if (reg_read(dev, ADDRESS_DEVICE_ID, &dev->reg_control[1], 1) !=
      FUSB302_SUCCESS) {
...
FUSB302_ret_t FUSB302_get_ID(FUSB302_dev_t *dev, uint8_t *version_ID,
                             uint8_t *revision_ID) {
...
FUSB302_ret_t FUSB302_get_message(FUSB302_dev_t *dev, uint16_t *header,
                                  uint32_t *data) {
  if (header) {
...
FUSB302_ret_t FUSB302_tx_sop(FUSB302_dev_t *dev, uint16_t header,
                             const uint32_t *data) {
  uint8_t buf[40];
  uint8_t *pbuf = buf;
...
FUSB302_ret_t FUSB302_alert(FUSB302_dev_t *dev, FUSB302_event_t *events) {
  FUSB302_ret_t (*const handler[])(FUSB302_dev_t *, FUSB302_event_t *) = {
      FUSB302_state_unattached, FUSB302_state_attached};
