...
void PD_UFP_c::init(uint8_t int_pin, enum PD_power_option_t power_option) { init_PPS(int_pin, 0, 0, power_option); }
  ...
  if (events & PD_PROTOCOL_EVENT_PS_RDY)
  {
    ...
    if (p.type == PD_PDO_TYPE_AUGMENTED_PDO)
    {
      ...
      else
      {
        ...
        status_power_ready(
                STATUS_POWER_PPS, PD_protocol_get_PPS_voltage(&protocol), PD_protocol_get_PPS_current(&protocol));
      }
    }
    else
    {
...
uint16_t PD_UFP_c::clock_ms(void) { return (uint16_t)millis() * clock_prescaler; }