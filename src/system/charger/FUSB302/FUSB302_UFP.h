...
typedef struct {
  /* setup by user */
  uint8_t i2c_address;
  FUSB302_ret_t (*i2c_read)(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data,
                            uint8_t count);
  FUSB302_ret_t (*i2c_write)(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data,
                             uint8_t count);
...
static inline const char *FUSB302_get_last_err_msg(FUSB302_dev_t *dev) {
  return dev->err_msg;
}
