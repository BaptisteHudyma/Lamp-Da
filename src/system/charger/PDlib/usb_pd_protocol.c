...
#ifdef CONFIG_USB_PD_REV30
int pd_get_rev(int port) { return pd[port].rev; }

int pd_get_vdo_ver(int port) { return vdo_ver[pd[port].rev]; }
#endif
 ...
 */
int pd_ts_dts_plugged(int port) { return pd[port].flags & PD_FLAGS_TS_DTS_PARTNER; }

#ifdef CONFIG_USB_PD_DUAL_ROLE
void pd_vbus_low(int port) { pd[port].flags &= ~PD_FLAGS_VBUS_NEVER_LOW; }
...
#if !defined(CONFIG_USB_PD_MAX_SINGLE_SOURCE_CURRENT) && defined(CONFIG_USB_PD_REV30)
      /* Restore Rp */
...
static void inc_id(int port) { pd[port].msg_id = (pd[port].msg_id + 1) & PD_MESSAGE_ID_COUNT; }
...
static inline void pd_ca_reset(int port) { pd[port].ca_buffered = 0; }
#endif
...
static int pd_transmit(int port, enum tcpm_transmit_type type, uint16_t header, const uint32_t* data)
{
  int evt;
   ...
   */
  if ((pd[port].rev == PD_REV30) && (pd[port].flags & PD_FLAGS_EXPLICIT_CONTRACT))
  {
    if (pd[port].power_role == PD_ROLE_SOURCE)
    {
      /*
    ...
    else if (type != TCPC_TX_HARD_RESET)
    {
      ...
      if (cc1 == TYPEC_CC_VOLT_SNK_1_5 || cc2 == TYPEC_CC_VOLT_SNK_1_5)
      {
        ...
        memcpy(pd[port].ca_buffer, data, sizeof(uint32_t) * PD_HEADER_CNT(header));
   ...
   */
  if ((pd[port].rev == PD_REV30) && (pd[port].power_role == PD_ROLE_SOURCE) &&
...
static void pd_ca_send_pending(int port)
{
  ...
  tcpm_get_cc(port, &cc1, &cc2);
  if ((cc1 != TYPEC_CC_VOLT_SNK_1_5) && (cc2 != TYPEC_CC_VOLT_SNK_1_5))
    if (pd_transmit(port, pd[port].ca_type, pd[port].ca_header, pd[port].ca_buffer) < 0)
      return;
...
{
  int bit_len;
  uint16_t header = PD_HEADER(type, pd[port].power_role, pd[port].data_role, pd[port].msg_id, 0, pd_get_rev(port), 0);
...
static int send_source_cap(int port)
{
  ...
  if (src_pdo_cnt == 0)
    ...
    header =
            PD_HEADER(PD_CTRL_REJECT, pd[port].power_role, pd[port].data_role, pd[port].msg_id, 0, pd_get_rev(port), 0);
  else
    header = PD_HEADER(PD_DATA_SOURCE_CAP,
                       pd[port].power_role,
                       pd[port].data_role,
                       pd[port].msg_id,
                       src_pdo_cnt,
...
{
  int bit_len;
  uint16_t header = PD_HEADER(PD_DATA_SINK_CAP,
                              pd[port].power_role,
                              pd[port].data_role,
                              pd[port].msg_id,
                              pd_snk_pdo_cnt,
...
static int send_request(int port, uint32_t rdo)
{
  int bit_len;
  uint16_t header =
          PD_HEADER(PD_DATA_REQUEST, pd[port].power_role, pd[port].data_role, pd[port].msg_id, 1, pd_get_rev(port), 0);
...
static int send_bist_cmd(int port)
{
  ...
  int bit_len;
  uint16_t header =
          PD_HEADER(PD_DATA_BIST, pd[port].power_role, pd[port].data_role, pd[port].msg_id, 1, pd_get_rev(port), 0);
...
static void queue_vdm(int port, uint32_t* header, const uint32_t* data, int data_cnt)
{
...
static void handle_vdm_request(int port, int cnt, uint32_t* payload)
{
  ...
  if (pd[port].vdm_state == VDM_STATE_BUSY)
  {
    ...
    {
      pd[port].vdm_timeout.val = get_time().val + PD_T_VDM_BUSY;
      pd[port].vdm_state = VDM_STATE_WAIT_RSP_BUSY;
      pd[port].vdo_retry = (payload[0] & ~VDO_CMDT_MASK) | CMDT_INIT;
      return;
    }
    else
    {
...
void pd_execute_hard_reset(int port)
{
   ...
   */
  if (pd[port].task_state == PD_STATE_SNK_SWAP_STANDBY || pd[port].task_state == PD_STATE_SNK_SWAP_COMPLETE)
  {
...
{
  pd[port].msg_id = 0;
  set_state(port, DUAL_ROLE_IF_ELSE(port, PD_STATE_SNK_DISCOVERY, PD_STATE_SRC_DISCOVERY));
...
static int pd_send_request_msg(int port, int always_send_request)
{
   ...
   */
  res = pd_build_request(port, &rdo, &curr_limit, &supply_voltage, PD_REQUEST_MAX);
...
#ifdef CONFIG_CHARGE_MANAGER
#ifdef CONFIG_USB_PD_ALT_MODE_DFP
  int charge_whitelisted = (pd[port].power_role == PD_ROLE_SINK &&
                            pd_charge_from_device(pd_get_identity_vid(port), pd_get_identity_pid(port)));
#else
...
static void handle_data_request(int port, uint16_t head, uint32_t* payload)
{
  ...
  switch (type)
  {
    ...
    case PD_DATA_BIST:
      ...
      if (DUAL_ROLE_IF_ELSE(port, pd[port].task_state == PD_STATE_SNK_READY, pd[port].task_state == PD_STATE_SRC_READY))
      {
        ...
        if ((payload[0] >> 28) == 5)
        {
          ...
          set_state(port, DUAL_ROLE_IF_ELSE(port, PD_STATE_SNK_DISCONNECTED, PD_STATE_SRC_DISCONNECTED));
        }
      }
      break;
...
{
  if (pd[port].task_state == PD_STATE_SRC_READY || pd[port].task_state == PD_STATE_SNK_READY)
...
static void pd_set_data_role(int port, int role)
{
   ...
   */
  if (role == PD_ROLE_DFP)
    usb_mux_set(port, TYPEC_MUX_USB, USB_SWITCH_CONNECT, pd[port].polarity);
  else
    usb_mux_set(port, TYPEC_MUX_NONE, USB_SWITCH_DISCONNECT, pd[port].polarity);
#else
  usb_mux_set(port, TYPEC_MUX_USB, USB_SWITCH_CONNECT, pd[port].polarity);
  ...
  switch (type)
  {
      ...
      else if (pd[port].task_state == PD_STATE_SNK_REQUESTED)
      {
        ...
        if (pd[port].flags & PD_FLAGS_EXPLICIT_CONTRACT)
        {
          ...
          if (type == PD_CTRL_WAIT)
          {
             ...
             */
            set_state_timeout(port, get_time().val + PD_T_SINK_REQUEST, PD_STATE_SNK_READY);
          }
          else
          {
    ...
    case PD_CTRL_PR_SWAP:
      ...
      if (pd_check_power_swap(port))
      {
        ...
        set_state(port, DUAL_ROLE_IF_ELSE(port, PD_STATE_SNK_SWAP_SNK_DISABLE, PD_STATE_SRC_SWAP_SNK_DISABLE));
      }
      else
      {
        send_control(port, REFUSE(pd[port].rev));
  ...
  switch (pd[port].vdm_state)
  {
    case VDM_STATE_READY:
      ...
      header = PD_HEADER(PD_DATA_VENDOR_DEF,
                         ...
                         pd_get_rev(port),
                         0);
      res = pd_transmit(port, TCPC_TX_SOP, header, pd[port].vdo_data);
      if (res < 0)
      {
      ...
      else
      {
        ...
        pd[port].vdm_timeout.val = get_time().val + vdm_get_ready_timeout(pd[port].vdo_data[0]);
      }
      break;
    ...
    case VDM_STATE_BUSY:
      ...
      if (pd[port].vdm_timeout.val && (get_time().val > pd[port].vdm_timeout.val))
      {
...
int pd_dev_store_rw_hash(int port, uint16_t dev_id, uint32_t* rw_hash, uint32_t current_image)
{
#ifdef CONFIG_COMMON_RUNTIME
  int i;
#endif
...
void pd_update_dual_role_config(int port)
{
   ...
   */
  if (pd[port].power_role == PD_ROLE_SINK && drp_state == PD_DRP_FORCE_SOURCE)
  {
...
int pd_get_role(int port) { return pd[port].power_role; }

static int pd_is_power_swapping(int port)
{
  /* return true if in the act of swapping power roles */
  return pd[port].task_state == PD_STATE_SNK_SWAP_SNK_DISABLE || pd[port].task_state == PD_STATE_SNK_SWAP_SRC_DISABLE ||
         pd[port].task_state == PD_STATE_SNK_SWAP_STANDBY || pd[port].task_state == PD_STATE_SNK_SWAP_COMPLETE ||
         pd[port].task_state == PD_STATE_SRC_SWAP_SNK_DISABLE || pd[port].task_state == PD_STATE_SRC_SWAP_SRC_DISABLE ||
...
static void pd_partner_port_reset(int port)
{
   ...
   */
  if (!pd_get_saved_active(port) || system_get_image_copy() != SYSTEM_IMAGE_RO ||
      system_get_reset_flags() & (RESET_FLAG_BROWNOUT | RESET_FLAG_POWER_ON))
    return;
#endif // CONFIG_BBRAM
...
int pd_get_polarity(int port) { return pd[port].polarity; }
...
{
  return (cc == TYPEC_CC_VOLT_SNK_DEF) || (cc == TYPEC_CC_VOLT_SNK_1_5) || (cc == TYPEC_CC_VOLT_SNK_3_0);
}
...
void pd_run_state_machine(int port, int reset)
{
  ...
  switch (this_state)
  {
    ...
    case PD_STATE_SRC_DISCONNECTED:
       ...
       */
      if (auto_toggle_supported && !(pd[port].flags & PD_FLAGS_TCPC_DRP_TOGGLE) &&
          !(pd[port].flags & PD_FLAGS_TRY_SRC) && (cc1 == TYPEC_CC_VOLT_OPEN && cc2 == TYPEC_CC_VOLT_OPEN))
      {
      ...
      /* Vnc monitoring */
      if ((cc1 == TYPEC_CC_VOLT_RD || cc2 == TYPEC_CC_VOLT_RD) || (cc1 == TYPEC_CC_VOLT_RA && cc2 == TYPEC_CC_VOLT_RA))
      {
       ...
       */
      else if ((pd[port].flags & PD_FLAGS_TRY_SRC && get_time().val >= pd[port].try_src_marker) ||
               (!(pd[port].flags & PD_FLAGS_TRY_SRC) && drp_state != PD_DRP_FORCE_SOURCE &&
                drp_state != PD_DRP_FREEZE && get_time().val >= next_role_swap))
      {
        pd[port].power_role = PD_ROLE_SINK;
        set_state(port, PD_STATE_SNK_DISCONNECTED);
        tcpm_set_cc(port, TYPEC_CC_RD);
      ...
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        set_state_timeout(port,
                          ...
                          get_time().val + PD_POWER_SUPPLY_TURN_ON_DELAY -
                                  (pd[port].last_state == PD_STATE_SRC_DISCONNECTED_DEBOUNCE ? PD_T_CC_DEBOUNCE : 0),
#else
                          get_time().val + PD_POWER_SUPPLY_TURN_ON_DELAY,
#endif
    ...
    case PD_STATE_SRC_ACCEPTED:
      ...
      if (pd[port].last_state != pd[port].task_state)
        set_state_timeout(port, get_time().val + PD_T_SINK_TRANSITION, PD_STATE_SRC_POWERED);
      break;
    case PD_STATE_SRC_POWERED:
      ...
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        set_state_timeout(port, get_time().val + PD_POWER_SUPPLY_TURN_ON_DELAY, PD_STATE_SRC_TRANSITION);
      }
      break;
    ...
    case PD_STATE_SRC_READY:
      ...
      if (pd[port].data_role == PD_ROLE_DFP && (pd[port].flags & PD_FLAGS_CHECK_IDENTITY))
      {
    ...
    case PD_STATE_SRC_GET_SINK_CAP:
      if (pd[port].last_state != pd[port].task_state)
        set_state_timeout(port, get_time().val + PD_T_SENDER_RESPONSE, PD_STATE_SRC_READY);
      break;
    case PD_STATE_DR_SWAP:
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        set_state_timeout(port, get_time().val + PD_T_SENDER_RESPONSE, READY_RETURN_STATE(port));
      }
      break;
#ifdef CONFIG_USB_PD_DUAL_ROLE
    ...
    case PD_STATE_SRC_SWAP_SNK_DISABLE:
      ...
      if (pd[port].last_state != pd[port].task_state)
        set_state_timeout(port, get_time().val + PD_T_SINK_TRANSITION, PD_STATE_SRC_SWAP_SRC_DISABLE);
      break;
    case PD_STATE_SRC_SWAP_SRC_DISABLE:
      ...
      {
        pd_power_supply_reset(port);
        set_state_timeout(port, get_time().val + PD_POWER_SUPPLY_TURN_OFF_DELAY, PD_STATE_SRC_SWAP_STANDBY);
      }
      break;
    case PD_STATE_SRC_SWAP_STANDBY:
      ...
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        set_state_timeout(port, get_time().val + PD_T_PS_SOURCE_ON, PD_STATE_SNK_DISCONNECTED);
      }
      break;
...
#else
        if (rstatus != EC_ERROR_UNIMPLEMENTED && pd_restart_tcpc(port) != 0)
        {
    ...
    case PD_STATE_SNK_HARD_RESET_RECOVER:
      ...
      if (pd[port].last_state != pd[port].task_state)
        set_state_timeout(port,
                          get_time().val + PD_T_SAFE_0V + PD_T_SRC_RECOVER_MAX + PD_T_SRC_TURN_ON,
    ...
    case PD_STATE_SNK_DISCOVERY:
      ...
      if ((pd[port].last_state != pd[port].task_state) && pd_comm_is_enabled(port))
      {
        ...
        if (pd[port].flags & PD_FLAGS_VBUS_NEVER_LOW)
          set_state_timeout(port, get_time().val + PD_T_SINK_WAIT_CAP, PD_STATE_SOFT_RESET);
        /*
         ...
         */
          ...
          /* ErrorRecovery */
          set_state_timeout(port, get_time().val + PD_T_NO_RESPONSE, PD_STATE_SNK_DISCONNECTED);
#if defined(CONFIG_CHARGE_MANAGER)
        /*
    ...
    case PD_STATE_SNK_TRANSITION:
      /* Wait for PS_RDY */
      if (pd[port].last_state != pd[port].task_state)
        set_state_timeout(port, get_time().val + PD_T_PS_TRANSITION, PD_STATE_HARD_RESET_SEND);
      break;
    case PD_STATE_SNK_READY:
      ...
      if (pd[port].flags & PD_FLAGS_CHECK_PR_ROLE)
      {
        pd_check_pr_role(port, PD_ROLE_SINK, pd[port].flags);
        pd[port].flags &= ~PD_FLAGS_CHECK_PR_ROLE;
        break;
      }
    ...
    case PD_STATE_SNK_SWAP_INIT:
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        /* Wait for accept or reject */
        set_state_timeout(port, get_time().val + PD_T_SENDER_RESPONSE, PD_STATE_SNK_READY);
      }
      break;
    ...
    case PD_STATE_SNK_SWAP_SRC_DISABLE:
      /* Wait for PS_RDY */
      if (pd[port].last_state != pd[port].task_state)
        set_state_timeout(port, get_time().val + PD_T_PS_SOURCE_OFF, PD_STATE_HARD_RESET_SEND);
      break;
    case PD_STATE_SNK_SWAP_STANDBY:
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        set_state_timeout(port, get_time().val + PD_POWER_SUPPLY_TURN_ON_DELAY, PD_STATE_SNK_SWAP_COMPLETE);
      }
      break;
    ...
    case PD_STATE_VCONN_SWAP_INIT:
      if (pd[port].last_state != pd[port].task_state)
      {
        if (!(pd[port].flags & PD_FLAGS_VCONN_ON))
        {
          ...
          tcpm_set_vconn(port, 1);
          set_state_timeout(port, get_time().val + PD_VCONN_SWAP_DELAY, PD_STATE_VCONN_SWAP_READY);
        }
        else
        {
          set_state_timeout(port, get_time().val + PD_T_VCONN_SOURCE_ON, READY_RETURN_STATE(port));
        }
      }
      break;
    case PD_STATE_VCONN_SWAP_READY:
      if (pd[port].last_state != pd[port].task_state)
      {
        ...
        else
        {
          ...
          tcpm_set_vconn(port, 0);
          pd[port].flags &= ~PD_FLAGS_VCONN_ON;
          set_state_timeout(port, get_time().val + PD_VCONN_SWAP_DELAY, READY_RETURN_STATE(port));
        }
      }
      break;
#endif /* CONFIG_USBC_VCONN_SWAP */
    ...
    case PD_STATE_HARD_RESET_SEND:
      ...
      if (pd[port].last_state == PD_STATE_SNK_DISCOVERY ||
          (pd[port].last_state == PD_STATE_SOFT_RESET && (pd[port].flags & PD_FLAGS_VBUS_NEVER_LOW)))
      {
        pd[port].flags &= ~PD_FLAGS_VBUS_NEVER_LOW;
        /*
      ...
      if (!hard_reset_sent)
      {
        ...
        if (pd[port].power_role == PD_ROLE_SOURCE)
        {
          set_state_timeout(port, get_time().val + PD_T_PS_HARD_RESET, PD_STATE_HARD_RESET_EXECUTE);
        }
        else
        {
    ...
    case PD_STATE_DRP_AUTO_TOGGLE:
      {
        ...
        if (cc1 == TYPEC_CC_VOLT_OPEN && cc2 == TYPEC_CC_VOLT_OPEN)
          ...
          next_state = PD_STATE_SNK_DISCONNECTED;
                 ...
                 (drp_state != PD_DRP_TOGGLE_OFF && drp_state != PD_DRP_FORCE_SINK))
   ...
   * a hard reset.
   */
  if (pd[port].power_role == PD_ROLE_SINK && !pd_is_vbus_present(port) &&
      pd[port].task_state != PD_STATE_SNK_HARD_RESET_RECOVER && pd[port].task_state != PD_STATE_HARD_RESET_EXECUTE)
  {
...
#endif
    pd[i].flags |= PD_FLAGS_CHECK_PR_ROLE | PD_FLAGS_CHECK_DR_ROLE;
...
void pd_send_hpd(int port, enum hpd_event hpd)
{
  ...
  data[0] = VDO_DP_STATUS((hpd == hpd_irq), /* IRQ_HPD */
                          ...
                          0,                /* power low */
                          0x2);
  pd_send_vdm(port, USB_SID_DISPLAYPORT, VDO_OPOS(opos) | CMD_ATTENTION, data, 1);
  /* Wait until VDM is done. */
...
void pd_request_source_voltage(int port, int mv)
{
  ...
  if (pd[port].task_state == PD_STATE_SNK_READY || pd[port].task_state == PD_STATE_SNK_TRANSITION)
  {
...
{
  if ((pd[port].task_state >= PD_STATE_SRC_NEGOCIATE) && (pd[port].task_state <= PD_STATE_SRC_GET_SINK_CAP))
  {
    pd[port].flags |= PD_FLAGS_UPDATE_SRC_CAPS;
    // getting rid of task stuff
    // task_wake(PD_PORT_TO_TASK_ID(port));
...
static int command_pd(int argc, char** argv)
{
  ...
  else if (!strncasecmp(argv[2], "info", 4))
  {
    ...
    ccprintf("\nImage %s\n", system_image_copy_t_to_string(pd[port].current_image));
  }
  ...
  else if (!strncasecmp(argv[2], "vdm", 3))
  {
    ...
    if (!strncasecmp(argv[3], "ping", 4))
    {
      ...
      pd_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_PING_ENABLE, &enable, 1);
    }
    else if (!strncasecmp(argv[3], "curr", 4))
    {
      pd_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_CURRENT, NULL, 0);
    }
    else if (!strncasecmp(argv[3], "vers", 4))
    {
      pd_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_VERSION, NULL, 0);
    }
    else
    {
      return EC_ERROR_PARAM_COUNT;
...
static int hc_usb_pd_control(struct host_cmd_handler_args* args)
{
  ...
  {
    r_v1->enabled = (pd_comm_is_enabled(p->port) ? PD_CTRL_RESP_ENABLED_COMMS : 0) |
    ...
    r_v1->role = (pd[p->port].power_role ? PD_CTRL_RESP_ROLE_POWER : 0) |
    ...
    strzcpy(r_v1->state, pd_state_names[pd[p->port].task_state], sizeof(r_v1->state));
...
static int hc_remote_flash(struct host_cmd_handler_args* args)
{
  ...
  switch (p->cmd)
  {
    ...
    case USB_PD_FW_FLASH_WRITE:
      ...
      {
        pd_send_vdm(port, USB_VID_GOOGLE, VDO_CMD_FLASH_WRITE, data + i, MIN(size - i, VDO_MAX_SIZE - 1));
        timeout.val = get_time().val + 500 * MSEC_US;
  ...
  if ((pd[port].vdm_state > 0) || (pd[port].vdm_state == VDM_STATE_ERR_TMOUT))
    rv = EC_RES_TIMEOUT;
  else if (pd[port].vdm_state < 0)
    rv = EC_RES_ERROR;
  ...
  switch (p->cmd)
  {
    case PD_EXIT_MODE:
      if (pd_dfp_exit_mode(p->port, p->svid, p->opos))
        pd_send_vdm(p->port, p->svid, CMD_EXIT_MODE | VDO_OPOS(p->opos), NULL, 0);
      else
      {
