...
{ //
  ...
  return waitForEvent(policy_engine_state::PESinkWaitCap,
                      (uint32_t)Notifications::MSG_RX | (uint32_t)Notifications::I_OVRTEMP |
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_select_cap()
{
  ...
  return waitForEvent(PESinkWaitCapResp,
                      (uint32_t)Notifications::MSG_RX | (uint32_t)Notifications::RESET |
                              (uint32_t)Notifications::TIMEOUT,
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_handle_epr_chunk()
{
  ...
  {
    memcpy(&(this->recent_epr_capabilities.data[chunk_index * PD_MAX_EXT_MSG_CHUNK_LEN]),
           &(tempMessage.data),
           2 + (4 * (PD_NUMOBJ_GET(&tempMessage) - 1)));
  }
  const auto recievedLength = (PD_MAX_EXT_MSG_CHUNK_LEN * chunk_index) /*Bytes Implicit by chunk index*/ +
                              2 /*half PDO*/ + (4 * (PD_NUMOBJ_GET(&tempMessage) - 1) /* Data in this message*/);
  ...
  tempMessage.exthdr =
          ((chunk_index + 1) << PD_EXTHDR_CHUNK_NUMBER_SHIFT) | PD_EXTHDR_REQUEST_CHUNK | PD_EXTHDR_CHUNKED;
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_wait_good_crc()
{
  ...
  while (incomingMessages.getOccupied())
  {
    ...
    if (PD_MSGTYPE_GET(&goodcrc) == PD_MSGTYPE_GOODCRC && PD_NUMOBJ_GET(&goodcrc) == 0 &&
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_wait_send_done()
{
  ...
  if ((uint32_t)evt & (uint32_t)Notifications::I_TXSENT)
  {
    ...
    {
      // No Good CRC has arrived, these should _normally_ come really fast (100us), but users implementation may be
      // lagging Setup a callback for this state
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_epr_eval_cap()
{
  ...
  if (pdbs_dpm_epr_evaluate_capability(&recent_epr_capabilities, &_last_dpm_request))
  {
    ...
    PPSTimerEnabled = (recent_epr_capabilities.obj[pps_index - 1] & PD_PDO_TYPE) == PD_PDO_TYPE_AUGMENTED &&
                      (recent_epr_capabilities.obj[pps_index - 1] & PD_APDO_TYPE) == PD_APDO_TYPE_PPS;
    _last_dpm_request.hdr |= hdr_template;
    return PESinkSelectCapTx;
  }
  else
...
PolicyEngine::policy_engine_state PolicyEngine::pe_sink_wait_epr_keep_alive_ack()
{
  ...
  {
    incomingMessages.pop(&tempMessage);
    if (PD_MSGTYPE_GET(&tempMessage) == PD_MSGTYPE_EXTENDED_CONTROL &&
        tempMessage.data[0] == PD_EXTENDED_CONTROL_TYPE_EPR_KEEPALIVE_ACK)
    {
      negotiationOfEPRInProgress = false;
      EPRTimeLastEvent = getTimeStamp();
      return PESinkReady;
