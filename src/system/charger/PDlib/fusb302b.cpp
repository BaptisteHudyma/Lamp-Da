...
{
  return (fusb_read_byte(FUSB_STATUS1) & FUSB_STATUS1_RX_EMPTY) != FUSB_STATUS1_RX_EMPTY;
}
...
bool FUSB302::fusb_setup() const
{
  ...
  /* Enable switchs & comparators */
  uint8_t REG_CNTR_0 = FUSB_SWITCHES0_PDWN_1 | FUSB_SWITCHES0_PDWN_2;
  if (!fusb_write_byte(FUSB_CONTROL0, REG_CNTR_0))
  {
    return false;
  }

  /* Enable automatic retransmission */
  uint8_t REG_CNTR_3 = FUSB_CONTROL3_N_RETRIES(3) | FUSB_CONTROL3_AUTO_RETRY;
  if (!fusb_write_byte(FUSB_CONTROL3, REG_CNTR_3))
  {
    return false;
  }
  // set defaults
  ...
  /* Turn on all power */
  if (!fusb_write_byte(FUSB_POWER, FUSB_POWER_PWR0 | FUSB_POWER_PWR1 | FUSB_POWER_PWR2))
  {
    return false;
  }
...
{
  /* Measure CC1 */
  if (!fusb_write_byte(FUSB_SWITCHES0, FUSB_SWITCHES0_PDWN_1 | FUSB_SWITCHES0_PDWN_2 | FUSB_SWITCHES0_MEAS_CC1))
  {
    return false;
  }
  osDelay(10);
  ...
  /* Measure CC2 */
  if (!fusb_write_byte(FUSB_SWITCHES0, FUSB_SWITCHES0_PDWN_1 | FUSB_SWITCHES0_PDWN_2 | FUSB_SWITCHES0_MEAS_CC2))
  {
    return false;
  }
  osDelay(10);
  ...
  if (cc1 > cc2)
  {
    ...
    if (!fusb_write_byte(FUSB_SWITCHES1, FUSB_SWITCHES1_TXCC1 | FUSB_SWITCHES1_AUTO_CRC | FUSB_SWITCHES1_SPECREV(0)))
    {
      return false;
    }
  ...
  else
  {
    ...
    if (!fusb_write_byte(FUSB_SWITCHES1, FUSB_SWITCHES1_TXCC2 | FUSB_SWITCHES1_AUTO_CRC | FUSB_SWITCHES1_SPECREV(0)))
    {
      return false;
    }
...
bool FUSB302::isVBUSOk() const
{
  const uint8_t regControl = fusb_read_byte(FUSB_STATUS0);
  return (regControl & FUSB_MASK1_M_VBUSOK) != 0x00;
}
...
void FUSB302::set_state_unattached()
{
  if (isVBUSOk())
  {
    fusb_reset();
...
{
  uint8_t data[1];
  while (!I2CRead(DeviceAddress, addr, 1, (uint8_t*)data))
  {
  }
  return data[0];
}
