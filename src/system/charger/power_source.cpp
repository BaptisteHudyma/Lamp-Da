...
#include "PDlib/fusb302_defines.h"
#include "PDlib/fusb302b.h"
#include "PDlib/policy_engine.h"
#include "src/system/charger/PDlib/policy_engine.h"
...
void pd_sink_capabalities(pd_msg* cap, const bool isPD3)
{
  ...
  cap->obj[numobj++] = PD_PDO_TYPE_FIXED | PD_PDO_SNK_FIXED_VOLTAGE_SET(PD_MV2PDV(5000)) |
  ...
  { // If requesting more than 5V
    ...
    cap->obj[numobj++] = PD_PDO_TYPE_FIXED | PD_PDO_SNK_FIXED_VOLTAGE_SET(PD_MV2PDV(voltage)) |
    ...
    if (isPD3)
    {
      cap->obj[numobj++] = PD_PDO_TYPE_AUGMENTED | PD_APDO_TYPE_PPS | PD_APDO_PPS_MAX_VOLTAGE_SET(PD_MV2PAV(voltage)) |
...
bool evaluate_capability(const pd_msg* capabilities, pd_msg* request)
{ /* Get the number of PDOs */
  ...
  if (bestIndex != 0xFF)
  {
    ...
    if (bestIsPPS)
    {
      request->obj[0] = PD_RDO_PROG_CURRENT_SET(PD_CA2PAI(bestIndexCurrent)) |
                        PD_RDO_PROG_VOLTAGE_SET(PD_MV2PRV(bestIndexVoltage)) | PD_RDO_NO_USB_SUSPEND |
    ...
    else
    {
      request->obj[0] = PD_RDO_FV_MAX_CURRENT_SET(bestIndexCurrent) | PD_RDO_FV_CURRENT_SET(bestIndexCurrent) |
                        PD_RDO_NO_USB_SUSPEND | PD_RDO_OBJPOS_SET(bestIndex + 1);
    }
    // USB Data
  ...
  else
  {
    ...
    request->hdr = PD_MSGTYPE_REQUEST | PD_NUMOBJ(1);
    request->obj[0] = PD_RDO_FV_MAX_CURRENT_SET(DPM_MIN_CURRENT) | PD_RDO_FV_CURRENT_SET(DPM_MIN_CURRENT) |
                      PD_RDO_NO_USB_SUSPEND | PD_RDO_OBJPOS_SET(1);
...
bool EPR_evaluate_capability(const epr_pd_msg* capabilities, pd_msg* request)
{
...
// initialize the USB pd negociation ic
FUSB302 fusb(FUSB302B_ADDR, writeI2cData, readI2cData, delay);

// initialize the policy engine (0 to EPR to not source more than 20V)
PolicyEngine policyEngine(fusb, millis, delay, pd_sink_capabalities, evaluate_capability, EPR_evaluate_capability, 0);
...
bool can_use_PD_full_power()
{
  ...
  // steps)
  return policyEngine.pdHasNegotiated(); // PD_UFP.get_vbus_voltage() > (PD_UFP.get_voltage() * 50 - 2000);
}
...
void loop()
{
  // run the negociation thread (with limited immediate rerun if needed)
  bool shouldRerun = true;
  for (uint8_t i = 0; i < 3 and shouldRerun; ++i)
  {
    shouldRerun = policyEngine.thread();
  }
  // run timer updates
  policyEngine.TimersCallback();
  // 500ms timeout before declaring that this is not a power delivery
  policyEngine.setupCompleteOrTimedOut(5);

  if (interruptSet)
  {
    interruptSet = false;
    policyEngine.IRQOccured();
  }

  // Serial.println(String(policyEngine.getStateName().c_str()));
...
uint16_t get_max_input_current()
{
  ...
  if (is_vbus_stable())
  {
    ...
    else
    {
      ...
      {
        // resolution of 10 mA
        return 1000; // PD_UFP.get_current() * 10;
      }
    }
  }
...
bool is_not_usb_power_delivery() { return policyEngine.isNotPowerDelivery(); }
