...
static float multAgc =
    1.0f;  // sample * multAgc = sampleAgc. Our AGC multiplier
static float sampleAvg = 0.3f;  // Smoothed Average sample - sampleAvg < 1 means
...
static uint8_t maxVol =
    31;  // Reasonable value for constant volume for 'peak detector', as it
...
static void detectSamplePeak(
    void);  // peak detection function (needs scaled FFT results in vReal[])
...
static float mapf(float x, float in_min, float in_max, float out_min,
                  float out_max);          // map function for float
static float fftAddAvg(int from, int to);  // average of several FFT result bins
void FFTcode();  // audio processing task: read samples, run FFT, fill GEQ
                 // channels from FFT results
...
// constexpr double SAMPLE_RATE = 22050;        // Base sample rate in Hz -
// 22Khz is a standard rate. Physical sample time -> 23ms
constexpr double SAMPLE_RATE = 16000;  // 16kHz - use if FFTtask takes more than
                                       // 20ms. Physical sample time -> 32ms
// constexpr double SAMPLE_RATE = 20480;        // Base sample rate in Hz -
// 20Khz is experimental.    Physical sample time -> 25ms constexpr double
// SAMPLE_RATE = 10240;        // Base sample rate in Hz - previous default.
...
static float vReal[samplesFFT] = {
    0.0f};  // FFT sample inputs / freq output -  these are our raw result bins
...
// ArduinoFFT to use sqrtf() instead of sqrt()
#define sqrt(x) \
  sqrtf(x)  // little hack that reduces FFT time by 10-50% on ESP32
#else
...
void FFTcode() {
  ...
  for (int i = 0; i < samplesFFT; i++) {
    ...
    vImag[i] = 0;
    // pick our  our current mic sample - we take the max value from all samples
    // that go into FFT
  ...
  if (sampleAvg > 0.25f) {  // noise gate open means that FFT results will be
    ...
    // Weigh data using "Blackman- Harris" window - sharp peaks due to excellent
    // sideband rejection
    FFT.compute(FFTDirection::Forward);  // Compute FFT
    FFT.complexToMagnitude();            // Compute magnitudes
#else
    FFT.DCRemoval();  // let FFT lib remove DC component, so we don't need to
                      // care about this in getSamples()

    // FFT.Windowing( FFT_WIN_TYP_HAMMING, FFT_FORWARD );        // Weigh data -
    // standard Hamming window FFT.Windowing( FFT_WIN_TYP_BLACKMAN, FFT_FORWARD
    // );       // Blackman window - better side freq rejection FFT.Windowing(
    // FFT_WIN_TYP_BLACKMAN_HARRIS, FFT_FORWARD );// Blackman-Harris - excellent
    // sideband rejection
    FFT.Windowing(FFT_WIN_TYP_FLT_TOP,
                  FFT_FORWARD);  // Flat Top Window - better amplitude accuracy
  ...
  if (fabsf(sampleAvg) > 0.5f) {  // noise gate open
    ...
    */                                    //  Range
    ...
    if (useBandPassFilter) {
      ...
      // don't use the last bins from 216 to 255. They are usually contaminated
      // by aliasing (aka noise)
      fftCalc[15] =
          fftAddAvg(165, 215) *
  ...
  for (int i = 0; i < numberOfChannels; i++) {
    ...
    if (fftCalc[i] > fftAvg[i])  // rise fast
      ...
      if (decayTime < 1000)
        ...
        fftAvg[i] = fftCalc[i] * 0.17f +
                    0.83f * fftAvg[i];  // default - approx  9 cycles (225ms)
                                        // for falling to zero
    ...
    switch (FFTScalingMode) {
      case 1:
        ...
        currentResult *=
            0.85f +
            (float(i) / 18.0f);  // extra up-scaling for high frequencies
      ...
      case 3:
        ...
        if (currentResult > 1.0f)
          ...
          currentResult =
              0.0f;  // special handling, because sqrt(0) = undefined
        currentResult *=
            0.85f + (float(i) / 4.5f);  // extra up-scaling for high frequencies
        currentResult =
            mapf(currentResult, 0.0, 16.0, 0.0,
...
static void autoResetPeak(void) {
  uint16_t MinShowDelay =
      50;  // Fixes private class variable compiler error. Unsure if this is the
           // correct way of fixing the root problem. -THATDONFC
...
static int16_t micIn = 0;  // Current sample starts with negative values and
                           // large values, which is why it's 16 bit signed
static double sampleMax =
    0.0;  // Max sample over a few seconds. Needed for AGC controller.
static float expAdjF = 0.0f;     // Used for exponential filter.
static float sampleReal = 0.0f;  // "sampleRaw" as float, to provide bits that
                                 // are lost otherwise (before amplification by
                                 // sampleGain or inputLevel). Needed for AGC.
static int16_t sampleRaw =
    0;  // Current sample. Must only be updated ONCE!!! (amplified mic value by
        // sampleGain and inputLevel)
...
const double agcFollowSlow[AGC_NUM_PRESETS] = {
    1 / 6144.f, 1 / 4096.f,
    1 / 8192.f};  // slowly follow setpoint  - ~2-15 secs
...
void processSample(int16_t sample) {
  ...
  if ((sampleMax < sampleReal) && (sampleReal > 0.5f)) {
    ...
    // another simple way to detect samplePeak - cannot detect beats, but reacts
    // on peak volume
