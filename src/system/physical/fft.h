 ...
 */
template <uint16_t samplesFFT, uint16_t fftResCount>
class FftAnalyzer {
 private:
  static constexpr unsigned useInputFilter =
      0;  // if >0 , enables a bandpass filter 80Hz-8Khz
          // to remove noise. Applies before FFT.

  static constexpr uint8_t inputLevel = 128;  // UI slider value

#ifndef SR_SQUELCH
  static constexpr uint8_t soundSquelch =
  ...
  static constexpr float multiplier =
      pow(maxFrequency / (float)minFrequency, 1 / (float)fftResCount);
  ...
  std::array<float, fftResCount>
      fftResultMax;  // A table used for testing to determine how
  ...
  float compute_linear_noise(uint16_t binIndex) {
    ...
    return 1.0 / (linearNoiseMultiplier * expf(linearNoiseExponentMultiplier *
                                               binIndex / (float)fftResCount));
  }
  ...
  void runHighFilter6db(const float filter, uint16_t numSamples,
                        float *sampleBuffer) {
    static float lowfilt = 0.0f;  // IIR low frequency cutoff filter
    for (int i = 0; i < numSamples; i++) {
  ...
  void runHighFilter12db(const float filter, uint16_t numSamples,
                         float *sampleBuffer) {
  ...
  ArduinoFFT<float> FFT = ArduinoFFT<float>(vReal, vImag, samplesFFT,
 ...
 public:
  ...
  void FFTcode() {
    ...
    for (int i = 0; i < samplesFFT;
         i++) {  // Values for bins 0 and 1 are WAY too
    ...
    for (uint16_t i = 0; i < fftResCount; ++i) {
      ...
      fftCalc[i] =
          fftCalc[i] - (float)soundSquelch * compute_linear_noise(i) / 4.0 <= 0
              ? 0
              : fftCalc[i];
      ...
      if (enableAutoGain)
        fftCalc[i] =
            fftCalc[i] * (float)sampleGain / 40.0 * inputLevel / 128.0 +
