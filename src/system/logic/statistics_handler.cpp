...
uint32_t get_system_on_time() { return statistics.system_on_minutes + max(static_cast<uint32_t>(1), time_s() / 60); }
...
uint32_t get_output_on_time()
{
  if (outputOn_time_s != UINT32_MAX)
  {
    ...
    if (currentTime_s >= outputOn_time_s)
      return statistics.output_on_minutes + max(static_cast<uint32_t>(1), (currentTime_s - outputOn_time_s) / 60);
    else
  ...
  if (batteryCharge_time_s != UINT32_MAX)
  {
    ...
    if (currentTime_s >= batteryCharge_time_s)
      return statistics.battery_charge_minutes +
             max(static_cast<uint32_t>(1), (currentTime_s - batteryCharge_time_s) / 60);
    else
