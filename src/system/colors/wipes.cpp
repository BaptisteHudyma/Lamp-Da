                   ...
                   const float cutOff)
{
  ...
  const unsigned long delay = max(MAIN_LOOP_UPDATE_PERIOD_MS, static_cast<uint32_t>(duration / (float)LED_COUNT));
...
bool color_vertical_wipe_right(const Color& color, const uint32_t duration, const bool restart, LedStrip& strip)
{
  ...
  // convert duration in delay for each segment
  const unsigned long delay = max(MAIN_LOOP_UPDATE_PERIOD_MS, static_cast<uint32_t>(duration / stripXCoordinates));
