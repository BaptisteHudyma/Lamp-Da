...
/// Lava color palette
const palette_t PaletteLavaColors = {
...
const palette_t PaletteForestColors = {
...
/// HSV Rainbow
const palette_t PaletteRainbowColors = {0xFF0000, 0xD52A00, 0xAB5500, 0xAB7F00,
                                        0xABAB00, 0x56D500, 0x00FF00, 0x00D52A,
                                        0x00AB55, 0x0056AA, 0x0000FF, 0x2A00D5,
                                        0x5500AB, 0x7F0081, 0xAB0055, 0xD5002B};

// basically, HSV with no green. looks better when lighing people
const palette_t PalettePartyColors = {0x5500AB, 0x84007C, 0xB5004B, 0xE5001B,
...
const palette_t PaletteHeatColors = {0x000000, 0x330000, 0x660000, 0x990000,
                                     0xCC0000, 0xFF0000, 0xFF3300, 0xFF6600,
                                     0xFF9900, 0xFFCC00, 0xFFFF00, 0xFFFF33,
                                     0xFFFF66, 0xFFFF99, 0xFFFFCC};

const palette_t PaletteAuroraColors = {0x000000, 0x003300, 0x006600, 0x009900,
                                       0x00cc00, 0x00ff00, 0x33ff00, 0x66ff00,
                                       0x99ff00, 0xccff00, 0xffff00, 0xffcc00,
                                       0xff9900, 0xff6600, 0xff3300, 0xff0000};

uint32_t get_color_from_palette(const uint8_t index, const palette_t& palette,
                                const uint8_t brightness) {
  const uint8_t renormIndex = index >> 4;  // convert to [0; 15] (divide by 16)
  const uint8_t blendIndex =
      index & 0x0F;  // mask with 15 (get the less significant part, that will
                     // be the blend factor)
  ...
  if (brightness != 255) {
    if (brightness != 0) {
      const float adjustedBirghtness =
...
uint32_t get_color_from_palette(const uint16_t index, const palette_t& palette,
                                const uint8_t brightness) {
