...
struct DoubleSideFillMode : public modes::BasicMode
{
  ...
  static void loop(auto& ctx)
  {
    ...
    const size_t endIndex = prog * (ctx.lamp.ledCount / 2.0f);
    ctx.lamp.fill(ctx.state.color, 0, endIndex);

    const size_t startIndex = ctx.lamp.ledCount / 2.0f + (1.0f - prog) * ctx.lamp.ledCount / 2.0f;
