...
inline const uint8_t* get_letter(const u_char letter)
{
    if(letter >= 32 and letter <= 243)
        // array starts at ascii 32
        return Font8x6[letter - 32];
    return Font8x6[1];  // return a random char
}


inline void display_text(const Color& color, const std::string& text, const uint16_t startXIndex, const uint16_t startYIndex, LedStrip& strip)
{
    uint8_t displayColumn = startXIndex;
    for(const char c : text)
    {
        const auto letter = get_letter(c);
        for(uint8_t i = 0; i <= 5; ++i)
        {
            // vertical binary mask
            uint8_t letterPart = letter[i];
            for(uint8_t y = 0; y < 8; ++y)
            {
                const bool shouldSet = (letterPart & 1) != 0;

                if (shouldSet)
                {
                    const auto pixelIndex = to_strip(displayColumn, startYIndex + y);
