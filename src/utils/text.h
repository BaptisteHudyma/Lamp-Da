...
//fonts from: http://www.piclist.com/techref/datafile/charset/extractor/charset_extractor.htm
...
struct SmallFont : public IFont
{
    ...
    inline uint8_t get_height() const override {
        return 8;
    };
    inline uint8_t get_width() const override {
        return 6;
    };
};
 ...
 */
uint8_t display_letter(const u_char letter, const int16_t startXIndex, const int16_t startYIndex, float scale, const Color& color, bool& isDisplayedOut, LedStrip& strip)
{
    uint8_t xIndex = 0;
    uint8_t yIndex = 0;
    ...
    bool hasCutoffX = false;        // letter was displayed out of screen, or cutout by the screen (above zero)
 ...
 */
bool display_text(const Color& color, const std::string& text, const int16_t startXIndex, const int16_t startYIndex, float scale, const bool paddEnd, LedStrip& strip)
{
...
/**
 * paddEnd: if true, will not consider the animation over until the last letter disappears from the strip (looks nicer)
 */
bool display_scrolling_text(const Color& color, const std::string& text, const int16_t startYIndex, float scale, const uint32_t duration, const bool reset, const bool paddEnd, LedStrip& strip)
{
    IFont const* font = font_from_scale(scale);
    ...
    if(duration / textLength < LOOP_UPDATE_PERIOD)
    {
        ...
        const uint32_t maxSubstep = LOOP_UPDATE_PERIOD / ((float)textLength / (float)duration * 1000.0);
